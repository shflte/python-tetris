...
mac_addr = "CD8600D38" + str(
    random.randint(100, 999)
)  # put here for easy to modify :-)
...
DAN.profile[
    "dm_name"
] = "Dummy_Device"  # you can change this but should also add the DM in server
DAN.profile["df_list"] = [
    "Dummy_Sensor",
    "Dummy_Control",
]  # Check IoTtalk to see what IDF/ODF the DM has
DAN.profile["d_name"] = (
    str(random.randint(100, 999)) + "_" + DAN.profile["dm_name"]
)  # None
...
threadx.start()
# create a socket to connect the tetris game
s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
# server binds on (('127.0.0.1', 9999))
s.connect(("127.0.0.1", 9876))

# send message to the tetris game according to the input from the IoTtalk
...
while True:
    try:
        ...
        if gotInput:  # 小弟有讀到資料了
            ...
            DAN.push(
                "Dummy_Sensor", value2, value2
            )  #  試這:  DAN.push('Dummy_Sensor', theInput)
